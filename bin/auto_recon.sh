...
echo $wan $ip
  if  [ $wan = $ip ]; then
  "${LIBERNET_DIR}/bin/log.sh" -w "<span style=\"color: Green\">Check Koneksi : OK [WAN = ${ip}]</span>"
    echo wan ok
    sleep 3
	n=0
  else
    echo ping fail
    n=$((n+1))
	"${LIBERNET_DIR}/bin/log.sh" -w "<span style=\"color: red\">Check Koneksi : Bengong Bos${n}</span>"
    sleep 3
  fi
  echo fail counter $n
  if [ $n -gt 3 ]; then
